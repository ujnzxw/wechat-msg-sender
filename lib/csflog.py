...
class _csflog(object):
    def __init__(self, logger_name="csflog", tag_name=""):
...
# csflog global hanlder print to STDOUT default
csflog = LogProxy(_csflog())
csflog.init_stream_handler()


def _test():
    csflog.init_logger("./test.log")

    csflog.debug("debug log")
    csflog.info("info %s log\n break line")
    try:
        raise Exception, "except exception"     # this is a exception comments
    except Exception:
        csflog.warning("warning %s", "log", exc_info=True)
    csflog.diagnose("error %slog", "format string")
    csflog.critical("critical log")
