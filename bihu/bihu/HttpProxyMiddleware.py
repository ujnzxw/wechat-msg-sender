...
logger = xzlog
#logger = logging.getLogger("xzlog")
...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, use_https):
        ...
        # 一个proxy如果被使用次数超过这个数字就强制更换下一个代理，防止被ban进行多余处理
        self.proxy_count_threshold = 500
        # 存放代理列表的文件, 每行一个代理, 格式为proto://ip:port, 这个文件会被修改, 注意备份
        self.proxy_file = "/root/bihu24h/bihu/bihu/proxies.dat"
        # 是否在超时的情况下禁用代理
        ...
        # 初始化代理列表
        #self.proxyes = [{"proxy": None, "valid": True, "count": 0},{"proxy": "https://89.22.175.42:8080", "valid": True, "count": 0}]
    ...
    def set_proxy(self, request):
        ...
        if not proxy["valid"] or proxy["count"] >= self.proxy_count_threshold:
        ...
        if proxy["proxy"]:
            ...
            print "=================="
            print "set proxy[%s] artid[%s]" % (request.meta["proxy"], request.meta["artid"])
            print "=================="
        ...
        print "=================="
        print "artid[%s] proxy_index[%s]" % (request.meta["artid"], request.meta["proxy_index"])
        print "=================="
        proxy["count"] += 1
    ...
    def process_exception(self, request, exception, spider):
        ...
        """
        pass
        #logger.debug("artid[%s] proxy[%s] exception: %s" % (request.meta["artid"], self.proxyes[request.meta["proxy_index"]]["proxy"], exception))
