...
def fetch_66ip(https):
    ...
    return proxies
def fetch_daxiang(https):
    proxies = []
    try:
        if https:
            url = 'http://tvp.daxiangdaili.com/ip/?tid=558483373903029&num=10&delay=1&category=2&protocol=https'
        else:
            url = 'http://tvp.daxiangdaili.com/ip/?tid=558483373903029&num=10&delay=1&category=2&protocol=http'
        content = get_html(url)
        content = str(content)
        urls = content.split('\r\n')
        for u in urls:
        ...
        logger.warning("fail to fetch from daxiang: %s" % e)
    return proxies
